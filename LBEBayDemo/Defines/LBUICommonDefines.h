...
/**  一些UI常用的方法   */
#ifndef LBUICommonDefines_h
#define LBUICommonDefines_h
...
/** 把数字转成字符串 */
CG_INLINE NSString *StringFromInteger(NSInteger number){
    return [NSString stringWithFormat:@"%zd",number];
}

/** 把float数字转成字符串 */
CG_INLINE NSString *StringFromOneFloat(CGFloat number){
    return [NSString stringWithFormat:@"%0.1f",number];
}

/** 判断range是否是有限的 */
CG_INLINE BOOL NSRangeIsValid(NSRange range){
